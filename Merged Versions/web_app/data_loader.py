"""
Extracts Sander + Emma source data, merges patients, and writes JSON files
into the data/ folder for the Flask app to serve.
"""
...
def load_or_generate():
    """Return merged data dicts.  Regenerate JSON cache if source files are newer."""
    paths = [
        os.path.join(DATA_DIR, "patients.json"),
        os.path.join(DATA_DIR, "survey_by_pid.json"),
        os.path.join(DATA_DIR, "cluster_survey.json"),
    ]
    source_mtime = max(os.path.getmtime(SANDER_HTML), os.path.getmtime(EMMA_JS))
    cache_ok = all(os.path.exists(p) for p in paths) and all(
        os.path.getmtime(p) >= source_mtime for p in paths
    )
    if cache_ok:
        ...
        with open(paths[2]) as f:
            cluster_survey = json.load(f)
        return merged, survey, cluster_survey
    ...
    for path, obj in zip(paths, [merged, survey_by_pid, cluster_survey]):
        with open(path, "w") as f:
            json.dump(obj, f, separators=(",", ":"))
    print("  JSON cache written to data/")
    return merged, survey_by_pid, cluster_survey
